...
# Models
from model.logic_net import LogicNet
...
session = Session(experiment='logic_net')

session.set_params(vars(args))
# 100, equation_split=train_split, negative_type='rd', train=False
train_split, test_split = train_test_splits()
train = MNISTAdd(args.path, args.dataset_size, train_split, 'rd', train=True)
valid = MNISTAdd(args.path, args.dataset_size, test_split, 'rd', train=False)
...
def train(epoch):
    ...
    acc_all = 0.0
    counter = 0
    for image_mat, target in tqdm.tqdm(trainloader):
        counter += 1
        image_mat = image_mat.float() / 255
        if args.cuda:
            image_mat = image_mat.cuda()
            target = target.cuda()
        loss, acc = model.train_(image_mat, None, target)
