...
class WReN(BasicModel):
    ...
    def forward(self, image, candidate):
        x = torch.cat([image, candidate], dim=1)  # B, 3*K + 8, C, H, W
        panel_features = self.conv(x.view(-1, 3, 28, 28))  # A little modification to the conv
        if self.use_tag:
